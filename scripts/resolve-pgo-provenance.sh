...
get() { (cd "$UPSTREAM" && ./rbm/rbm showconf firefox "$1" "${args[@]}") | tail -n1; }
...
git -C "$RUNNER_TEMP/firefox-source" fetch --depth=1 origin "refs/tags/$firefox_ref:refs/tags/$firefox_ref"
...
printf 'FIREFOX_REPOSITORY=%s\nFIREFOX_REF=%s\nFIREFOX_REVISION=%s\n' "$firefox_repo" "$firefox_ref" "$firefox_revision" >> "$GITHUB_ENV"
